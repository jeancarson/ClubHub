...
def display_event_details():
    ...
    for event in limited_upcoming_events:
        number_of_approved_participants = dbf.query_db(
            ecd.count_approved_participants.format(event_id=event["event_id"]))
        number_of_pending_participants = dbf.query_db(ecd.count_pending_participants.format(event_id=event["event_id"]))
        return render_template("html/coordinator/coordinator-dashboard.html", coordinator_name=coordinator_name,
                               club_id=club_id, club_title="Club 1", club_description="Description 1",
                               upcoming_events=limited_upcoming_events,
...
def edit_event():
    ...
    return render_template("html/coordinator/single-event-view.html", event_name='Event 1', event_date='2021-10-10',
...
#     dbf.modify_db(ecd.save_club_details.format(club_id=club_id, club_name=club_name, club_description=club_description))
#     return render_template("html/coordinator/coordinator-dashboard.html", coordinator_name=coordinator_name, club_id = club_id, club_title = club_name, club_description = club_description)
#
