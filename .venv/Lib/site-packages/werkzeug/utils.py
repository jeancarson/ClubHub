...
def get_content_type(mimetype: str, charset: str) -> str:
    ...
    .. versionchanged:: 0.15
        ...
        ``application/js`` are also given charsets.
    """
    if (
...
def find_modules(
    ...
    """Finds all the application below a package.  This can be useful to
    automatically import all views / controllers so that their metaclasses /
    function decorators have a chance to register themselves on the
    application.

    Packages are not returned unless `include_packages` is `True`.  This can
    also recursively list application but in that case it will import all the
    packages to get the correct load path of that module.

    :param import_path: the dotted name for the package to find child application.
